...
1. Ensure the component has a kind (`SetKind`) and ID (`SetID` when missing).
...
2. Client script scans for nodes with `data-flux-component-kind` and `data-flux-component-id` attributes.
3. Client reads component metadata and keeps the SSR DOM as-is.
